...
import { artData, data } from './mapState.js'
...
const startApp = () => {
  const { tileData: { tiles, tilesList, tileSheetData, tileX, tileY }, mapData } = data
  ...
  if (queryArray.length > 2){
    artData.column = +queryArray[2]
    artData.row = +queryArray[3]
    artData.d = +queryArray[4]
    input.row.value = artData.row
    input.column.value = artData.column
    input.d.value = artData.d

    artData.tiles = decompress(queryArray[5].replaceAll('-',','))
    input.codesBox[0].value = artData.tiles
    if (queryArray[6]) input.indexIndicator.value = queryArray[6]
    if (queryArray[7]) input.codesBox[2].value = decompress(mapData[queryArray[7]].walls)
  ...
  mapLinkCells.forEach((link, i)=>{
    link.addEventListener('click',()=>{
      ...
      const url = `${filePath}#${column}#${row}#${artData.d}#${map.replaceAll(',','-')}#${i}#${mapKeys[i]}`
